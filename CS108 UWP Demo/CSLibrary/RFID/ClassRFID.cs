        ...
        bool R2000Packet_Inventory (byte [] recvData, int offset = 0)
        {
            if (OnAsyncCallback != null)
            {
                ...
                if (_tagRangingParms.multibanks == 2)
				{
					...
					//Trace.Message("data 1:" + Tools.Hex.ToString(data));
				...
				if (_tagRangingParms.multibanks > 0)
				{
					...
					Debug.WriteLine ("data 2:" + Tools.Hex.ToString(data));
                    //Trace.Message("data 2:" + Tools.Hex.ToString(data));
                    ...
                    case Operation.TAG_SEARCHING:
                        {
                            ...
                            else
                            {
                                ...
                                //Trace.Message("Found a non-match Tag");
                            }
                        }
                        break;
 ...
 * 							 * if (m_Result == Result.OK && !Options.TagBlockLock.setPermalock)
		...
		internal CSLibrary.HighLevelInterface.BTWAITCOMMANDRESPONSETYPE DeviceRecvData(byte[] recvData1, HighLevelInterface.BTWAITCOMMANDRESPONSETYPE currentCommandResponse)
		{
            ...
            {
                Debug.WriteLine("RFID ring buffer FULL!!!!");
                //Trace.Message("RFID ring buffer FULL!!!!");
            }
			...
			while (_dataBuffer.length >= 8 )
			{
				...
				switch (header)
				{
                    default:
                        _dataBuffer.Clear();
                        Debug.WriteLine("Can not handle R2000 packet type :0x" + header.ToString("X2"));
                        //Trace.Message("Can not handle R2000 packet type :0x", header.ToString("X2"));
                        break;
											...
											case CSLibrary.Constants.Operation.TAG_LOCK:
												{
                                                    ...
                                                    Debug.WriteLine("Tag lock end {0}", currentCommandResponse);
                                                    //Trace.Message("Tag lock end {0}", currentCommandResponse);
...
#region ====================== Set Tag Group ======================
		...
		{
			Debug.WriteLine ("DateTime {0}" + DateTime.Now.ToString());
            //Trace.Message("DateTime {0}", DateTime.Now);

            _deviceHandler.SendAsync(0, 0, DOWNLINKCMD.RFIDPOWEROFF);
		}

        internal const UInt32 _INVENTORYDELAYTIME = (30 << 20);
