...
namespace CSLibrary {
    public partial class HighLevelInterface {
        ...
        private void CharacteristicOnValueUpdated(byte[] recvData) {
            CSLibrary.Debug.WriteBytes("BT data received", recvData);
        ...
        bool FirstAssemblePacketMohod(byte[] recvData) {
            if (CheckAPIHeader(recvData)) {
                if (_currentRecvBufferSize > 0) {
                    Debug.WriteLine("BT1 : Packet Too small, can not process");
                }
            ...
            if ((_currentRecvBufferSize + recvData.Length) > _recvBuffer[2] + 8) {
                Debug.WriteLine("BT1 : Current packet size too large");
                _currentRecvBufferSize = 0;
                return false;
            }
            ...
            if (_currentRecvBufferSize == (_recvBuffer[2] + 8)) {
                ...
                if (recvCRC != calCRC) {
                    Debug.WriteLine("BT1 : Checksum error " + recvCRC.ToString("X4") + " " + calCRC.ToString("X4"));
                    _currentRecvBufferSize = 0;
                    return false;
                }
        ...
        bool BackupAssemblePacketMohod(byte[] recvData) {
            if (_currentRecvBufferSizeBackup == 0) {
                if (!CheckAPIHeader(recvData)) return false;
            ...
            {
                Debug.WriteLine("BT2 : Current packet size too large");
                //Trace.Message("BT2 : Current packet size too large");
                _currentRecvBufferSizeBackup = 0;
                return false;
            }
            ...
            if (_currentRecvBufferSizeBackup == (_recvBuffer[2] + 8))
            {
                ...
                UInt16 calCRC = Tools.Crc.ComputeChecksum(_recvBuffer);
                if (recvCRC != calCRC) {
                    Debug.WriteLine("BT2 : Checksum error " + recvCRC.ToString("X4") + " " + calCRC.ToString("X4"));
                    //Trace.Message("BT2 : Checksum error " + recvCRC.ToString("X4") + " " + calCRC.ToString("X4"));
                    _currentRecvBufferSizeBackup = 0;
                    return false;
                }
