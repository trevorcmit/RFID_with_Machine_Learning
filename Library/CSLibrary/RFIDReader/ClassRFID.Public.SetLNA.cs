...
namespace CSLibrary {
    public partial class RFIDReader {
        ...
        public Result SetLNA(int rflna_high_comp, int rflna_gain, int iflna_gain, int ifagc_gain) {
            if (rflna_high_comp != 00 && rflna_high_comp != 1) return Result.INVALID_PARAMETER;

            if (rflna_gain != 1 && rflna_gain != 7 && rflna_gain != 13) return Result.INVALID_PARAMETER;

            if (iflna_gain != 6 && iflna_gain != 12 && iflna_gain != 18 && iflna_gain != 24) return Result.INVALID_PARAMETER;

            if (ifagc_gain != -12 && ifagc_gain != -6 && ifagc_gain != 0 && ifagc_gain != 6) return Result.INVALID_PARAMETER;
