...
namespace Plugin.BLE.Android {
    public class Descriptor : DescriptorBase {
        ...
        private void InternalWrite(byte[] data) {
            if (!_nativeDescriptor.SetValue(data)) throw new Exception("GATT: SET descriptor value failed");

            if (!_gatt.WriteDescriptor(_nativeDescriptor)) throw new Exception("GATT: WRITE descriptor value failed");
        }
        ...
        private void ReadInternal() {
            if (!_gatt.ReadDescriptor(_nativeDescriptor)) throw new Exception("GATT: read characteristic FALSE");
        }
    }
}